...
if [ ! -f "/preboot/kernel/${GENTOO_ARCH}/config-${KERNEL_VERSION}-gentoo" ]; then
	echo "Kernel configuration /preboot/kernel/${GENTOO_ARCH}/config-${KERNEL_VERSION}-gentoo was not found. Cannot continue." >&2
	exit 1
fi
rm -f "${KBUILD_OUTPUT}/.config"
ln -s "/preboot/kernel/${GENTOO_ARCH}/config-${KERNEL_VERSION}-gentoo" "${KBUILD_OUTPUT}/.config"
...
#cp --archive "/preboot/initramfs/fs.${GENTOO_ARCH}"/* "/cache/${KERNEL_SLUG}/initramfs/"

CPIO_LIST=$(mktemp)
cat "/preboot/initramfs/initramfs_list.${GENTOO_ARCH}" >> "${CPIO_LIST}"
echo >> "${CPIO_LIST}"
